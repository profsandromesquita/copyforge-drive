...
-- Fix trigger to extract imageUrl from correct path: block.config.imageUrl
...
BEGIN
  ...
  IF first_session IS NOT NULL THEN
    ...
    IF first_block IS NOT NULL THEN
      -- Extrair preview_image_url do caminho correto: config.imageUrl
      -- Ignorar base64 para performance
      IF first_block->>'type' = 'image' 
         AND first_block->'config'->>'imageUrl' IS NOT NULL 
         AND first_block->'config'->>'imageUrl' NOT LIKE 'data:%' THEN
        NEW.preview_image_url := first_block->'config'->>'imageUrl';
      ELSE
        -- Procurar em outros blocos por imagem (caminho correto: config.imageUrl)
        SELECT b->'config'->>'imageUrl' INTO NEW.preview_image_url
        FROM jsonb_array_elements(first_session->'blocks') AS b
        WHERE b->>'type' = 'image' 
          AND b->'config'->>'imageUrl' IS NOT NULL
          AND b->'config'->>'imageUrl' NOT LIKE 'data:%'
        LIMIT 1;
      END IF;
