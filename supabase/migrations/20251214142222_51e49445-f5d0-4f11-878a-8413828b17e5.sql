CREATE OR REPLACE FUNCTION public.change_workspace_plan(p_workspace_id uuid, p_plan_offer_id uuid, p_payment_id text DEFAULT NULL::text)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  v_is_super_admin := has_system_role(auth.uid(), 'super_admin');
  
  -- Verificar se é admin do workspace OU super admin
  IF NOT (is_workspace_admin(auth.uid(), p_workspace_id) OR v_is_super_admin) THEN
  ...
  -- Super admin não precisa de pagamento, ativa direto
  IF v_is_super_admin THEN
    v_requires_payment := false;
  ELSE
    v_requires_payment := (v_offer.price > 0) AND (p_payment_id IS NULL);
  END IF;
  ...
  INSERT INTO workspace_subscriptions (
    ...
    p_plan_offer_id,
    v_billing_cycle,
    CASE WHEN v_requires_payment THEN 'pending_payment'::subscription_status ELSE 'active'::subscription_status END,
