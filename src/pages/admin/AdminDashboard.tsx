...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const [clientesRes, workspacesRes, copiesRes, creditsRes, lastTransactionRes, modelStatsRes] = await Promise.all([supabase.from('profiles').select('id', {
          count: 'exact',
          head: true
        }), supabase.from('workspaces').select('id', {
          count: 'exact',
          head: true
        }), supabase.from('copies').select('id', {
          count: 'exact',
          head: true
        }), supabase.from('workspace_credits').select('balance, total_used'), supabase.from('ai_generation_history').select('credits_debited, total_tokens, tpc_snapshot, created_at').order('created_at', {
          ascending: false
        }).limit(1).single(), supabase.from('credit_transactions').select('model_used, tokens_used, input_tokens, output_tokens, amount').eq('transaction_type', 'debit').not('model_used', 'is', null)]);
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
          {cards.map(card => <Card key={card.title}>
              ...
              <CardContent>
                {loading ? <Skeleton className="h-8 w-20" /> : <div className="text-xl font-bold break-all">{card.value.toLocaleString()}</div>}
              </CardContent>
            </Card>)}
          ...
          <Card>
            ...
            <CardContent>
              {loading ? <Skeleton className="h-8 w-20" /> : <div className="text-xl font-bold break-all">{stats.saldo_total.toFixed(2)}</div>}
            </CardContent>
          </Card>
        </div>

        {/* Última Transação */}
        {lastTransaction && !loading && <CreditTransactionCard debited={lastTransaction.debited} tokens={lastTransaction.tokens} tpc={lastTransaction.tpc} timestamp={lastTransaction.timestamp} />}
