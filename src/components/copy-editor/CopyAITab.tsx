...
export const CopyAITab = ({
  ...
  const {
    characteristics: frameworksData,
    isLoading: loadingFrameworks
  } = useAICharacteristics('frameworks');
  const {
    characteristics: objetivosData,
    isLoading: loadingObjetivos
  } = useAICharacteristics('objetivos');
  const {
    characteristics: estilosData,
    isLoading: loadingEstilos
  } = useAICharacteristics('estilos');
  const {
    characteristics: focoEmocionalData,
    isLoading: loadingFocoEmocional
      ...
      if (error) {
        ...
        try {
          if (error.context?.body) {
            errorDetails = typeof error.context.body === 'string' ? JSON.parse(error.context.body) : error.context.body;
          }
        } catch (e) {
  ...
  const handleOptimizeGenerate = async () => {
    ...
    try {
      ...
      const {
        data: copyData
      } = await supabase.from('copies').select('workspace_id, project_id').eq('id', copyId).single();
      ...
      if (copyData.project_id) {
        const {
          data: projectData
        } = await supabase.from('projects').select('*').eq('id', copyData.project_id).single();
        if (projectData) {
          projectIdentity = {
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke('optimize-copy', {
        body: {
  ...
  const renderCriarTab = () => {
    ...
    if (etapa === 2) {
      return <ScrollArea className="h-[calc(100vh-12rem)]">
          <div className="space-y-5 p-4 animate-fade-in">
            <Button variant="ghost" onClick={() => setEtapa(1)} className="group -ml-2 px-2 text-muted-foreground hover:text-foreground transition-colors" size="sm">
            ...
            <div className="space-y-2">
              ...
              <Select value={estrutura} onValueChange={setEstrutura}>
                ...
                <SelectContent>
                  {FRAMEWORKS.map(framework => <SelectItem key={framework.value} value={framework.value}>
                      <div className="flex flex-col">
                        <span className="font-medium">{framework.label}</span>
                        {framework.description && <span className="text-xs text-muted-foreground">{framework.description}</span>}
                      </div>
                    </SelectItem>)}
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              ...
              <Select value={objetivo} onValueChange={setObjetivo}>
                ...
                <SelectContent>
                  {OBJETIVOS.map(obj => <SelectItem key={obj.value} value={obj.value}>
                      {obj.label}
                    </SelectItem>)}
                </SelectContent>
              </Select>
            ...
            <div className="space-y-2">
              ...
              <ToggleGroup type="multiple" value={estilos} onValueChange={setEstilos} className="flex flex-wrap gap-2 justify-start">
                {ESTILOS.map(estilo => <ToggleGroupItem key={estilo.value} value={estilo.value} className="rounded-full px-4 data-[state=on]:bg-primary/10 data-[state=on]:text-primary data-[state=on]:border-primary/20 transition-all hover-scale">
                    {estilo.label}
                  </ToggleGroupItem>)}
              </ToggleGroup>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select value={focoEmocional} onValueChange={setFocoEmocional}>
                ...
                <SelectContent>
                  {FOCO_EMOCIONAL.map(foco => <SelectItem key={foco.value} value={foco.value}>
                      <div className="flex flex-col">
                        <span className="font-medium">{foco.label}</span>
                        {foco.description && <span className="text-xs text-muted-foreground">{foco.description}</span>}
                      </div>
                    </SelectItem>)}
                </SelectContent>
              </Select>
    ...
    return <ScrollArea className="h-[calc(100vh-12rem)]">
        <div className="space-y-8 p-6 animate-fade-in">
          <Button variant="ghost" onClick={() => setEtapa(2)} className="group -ml-2 px-2 text-muted-foreground hover:text-foreground transition-colors" size="sm">
            <span className="transition-transform group-hover:-translate-x-1 inline-block">←</span>
            <span className="ml-2">Voltar</span>
          </Button>
            ...
            <div className="flex items-center justify-between">
              <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">
                Detalhes da Copy
                    ...
                    <Button variant="outline" size="sm" onClick={() => setShowGeneratedPromptEditor(true)} disabled={!generatedSystemPrompt} className={cn("gap-2 text-xs transition-all", generatedSystemPrompt && "animate-pulse ring-2 ring-offset-2 ring-amber-300/50 hover:animate-none hover:ring-primary/50")}>
                <Wand2 className="h-3.5 w-3.5" />
                {generatedSystemPrompt ? 'Editar System Prompt' : 'Aguarde geração...'}
              </Button>
            </div>
            <div className="relative">
                  <Textarea value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="Ex: 'Escreva uma sequência de 3 e-mails para Black Friday focado em urgência'. (Se você selecionou Público-Alvo ou Oferta, não precisa repetir!)" rows={14} className="resize-none rounded-2xl pr-12 border-2 border-border/60" />
          ...
          <Button onClick={handleGenerate} disabled={!prompt.trim() || isGenerating} className="w-full rounded-full shadow-sm hover:shadow-md transition-shadow">
            {isGenerating ? <>
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                Gerando...
              </> : 'Gerar Copy com IA'}
  ...
  return <>
      ...
      {loadingAccess ? <div className="flex items-center justify-center h-[calc(100vh-12rem)]">
          <Loader2 className="h-8 w-8 animate-spin text-primary" />
        </div> : !copyAIEnabled ? <div className="flex items-center justify-center h-[calc(100vh-12rem)]">
        ...
        </div> : isLoadingCharacteristics ? <div className="flex items-center justify-center h-[calc(100vh-12rem)]">
      ...
      <AIGeneratedPreviewModal open={showPreviewModal} onOpenChange={handleModalClose} generatedSessions={generatedSessions} onClose={handleModalClose} onSuccess={handleSuccess} />

      <SelectContentModal open={showSelectModal} onOpenChange={setShowSelectModal} sessions={copySessions} onConfirm={handleContentSelect} />

      <OptimizeComparisonModal open={showComparisonModal} onOpenChange={setShowComparisonModal} originalContent={selectedContent?.sessions || []} generatedContent={optimizedSessions} onReplace={handleReplace} onAdd={handleAdd} onRegenerate={handleRegenerate} isRegenerating={isOptimizing} />
      
      <SystemPromptEditorModal open={showPromptEditor} onClose={() => setShowPromptEditor(false)} copyType={copyType as any} />
      ...
      <Dialog open={showGeneratedPromptEditor} onOpenChange={setShowGeneratedPromptEditor}>
        <DialogContent className="max-w-3xl max-h-[85vh]">
          ...
          <Textarea value={generatedSystemPrompt || ''} onChange={e => setGeneratedSystemPrompt(e.target.value)} rows={20} className="font-mono text-sm" />
          <div className="flex justify-end gap-2">
            ...
            <Button onClick={() => {
            toast({
              title: 'System Prompt atualizado!'
            });
