        ...
        <>
          <div className="space-y-2 mb-3">
            ...
            <RadioGroup value={effectiveModel} onValueChange={handleModelChange}>
              ...
              {/* Opção GPT-5 */}
              <label htmlFor="gpt5" className={`flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition-all ${effectiveModel === 'openai/gpt-5' ? 'border-primary bg-primary/5' : 'border-muted hover:border-primary/50'}`}>
                <RadioGroupItem value="openai/gpt-5" id="gpt5" />
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
          ...
          {isDifferentFromRecommended && (
            <div className="flex items-start gap-2 p-2 bg-yellow-50 dark:bg-yellow-950/20 rounded border border-yellow-200 dark:border-yellow-900 text-xs mb-2">
              <Info className="h-4 w-4 text-yellow-600 dark:text-yellow-500 mt-0.5 flex-shrink-0" />
              <div>
                <p className="font-medium text-yellow-800 dark:text-yellow-300">
                  {selectedModel === 'google/gemini-2.5-flash' ? (
                    <>Economizando créditos, mas qualidade pode ser inferior para {getCopyTypeName()}</>
                  ) : (
                    <>Usando modelo premium quando o econômico seria suficiente - consumo maior de créditos</>
                  )}
                </p>
              </div>
            </div>
