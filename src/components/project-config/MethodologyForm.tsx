...
import { Lightbulb, Info } from 'phosphor-react';
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
export const MethodologyForm = ({ editingMethodology, onCancel, onSuccess }: MethodologyFormProps) => {
  ...
  const handleSave = async () => {
    ...
    if (!formData.name || formData.name.trim().length < 3) {
      toast.error('Informe um nome para a metodologia com pelo menos 3 caracteres');
      return;
    }
    ...
    setSaving(true);
    try {
      const currentMethodologies = Array.isArray(activeProject.methodology) 
        ? activeProject.methodology 
        : [];
      let updatedMethodologies;

      if (editingMethodology) {
        // Atualizar metodologia existente
        updatedMethodologies = currentMethodologies.map((m: Methodology) =>
          m.id === editingMethodology.id ? { ...formData, id: editingMethodology.id } : m
        );
      } else {
        // Adicionar nova metodologia
        const newMethodology: Methodology = {
          ...formData as Methodology,
          id: crypto.randomUUID(),
        };
        updatedMethodologies = [...currentMethodologies, newMethodology];
      }
      ...
      toast.success(editingMethodology ? 'Metodologia atualizada!' : 'Metodologia criada!');
      onSuccess();
    } catch (error) {
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold">
            {editingMethodology ? 'Editar Metodologia' : 'Criar Metodologia'}
          </h2>
        ...
        <CardContent className="space-y-6">
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Ex: Método de Emagrecimento Sustentável"
