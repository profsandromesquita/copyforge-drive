    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 py-4">
          ...
          <div><Label>Descrição curta *</Label><Textarea value={formData.short_description} onChange={e => setFormData({...formData, short_description: e.target.value})} rows={2} /></div>
          <div><Label>Benefício principal *</Label><Textarea value={formData.main_benefit} onChange={e => setFormData({...formData, main_benefit: e.target.value})} rows={2} /></div>
          <div><Label>Mecanismo único *</Label><Textarea value={formData.unique_mechanism} onChange={e => setFormData({...formData, unique_mechanism: e.target.value})} rows={2} /></div>
          ...
          <div><Label>Prova/Autoridade</Label><Textarea value={formData.proof} onChange={e => setFormData({...formData, proof: e.target.value})} rows={2} /></div>
          <div><Label>Garantia (opcional)</Label><Textarea value={formData.guarantee} onChange={e => setFormData({...formData, guarantee: e.target.value})} rows={2} /></div>
